...
ws.onmessage = (event) => {
  ...
  if (message.type === 'enter_success') {
  ...
  } else if (message.type === 'enter_error') {
    alert(message.error);
  } else if (message.type === 'question_created') {
    addQuestionToUI(
      {
        id: message.question.questionId,
        author: message.question.username,
        text: message.question.question,
        yes: message.question.yesNum,
        no: message.question.noNum
      },
      () => sendMessage({
        type: 'vote_question',
    ...
    updateVoteCountUI({
      questionId: message.question.questionId,
      answer: message.question.answer,
      count: message.question.answer === 'yes' ? message.question.yesNum : message.question.noNum
    })
  }
};